...
class AD_Standard_3DRandomPatch(Dataset):
    ...
    def __getitem__(self, idx):
        with open(self.data_file) as df:
            ...
            #print('333333333333333',patch_samples)
...
def getRandomPatches(image_array):
    ...
    #print('filtttttttttttterrrrrrrrrrrrrr : ',list(filter(lambda x: x>0, mean_ax)))
    first_ax = int(round(list(mean_ax).index(list(filter(lambda x: x>0, mean_ax))[0])))
    last_ax = int(round(list(mean_ax).index(list(filter(lambda x: x>0, mean_ax))[-1])))
    first_cor = int(round(list(mean_cor).index(list(filter(lambda x: x>0, mean_cor))[0])))
    last_cor = int(round(list(mean_cor).index(list(filter(lambda x: x>0, mean_cor))[-1])))
    first_sag = int(round(list(mean_sag).index(list(filter(lambda x: x>0, mean_sag))[0])))
    last_sag = int(round(list(mean_sag).index(list(filter(lambda x: x>0, mean_sag))[-1])))

    first_ax = first_ax + 20
    last_ax = last_ax - 5

    ax_samples = [random.randint(first_ax - 3, last_ax - 3) for r in range(10000)]
    cor_samples = [random.randint(first_cor - 3, last_cor - 3) for r in range(10000)]
    sag_samples = [random.randint(first_sag - 3, last_sag - 3) for r in range(10000)]
