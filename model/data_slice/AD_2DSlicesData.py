...
def getSlice(image_array, mean_direc, fraction, section):
    ...
    first_p = list(mean_array).index(next(lambda x: x>0, mean_array)[0])
    last_p = list(mean_array).index(next(lambda x: x>0, mean_array)[-1])
...
def getPackedSlices(image_array, mean_direc, fraction, section):
    ...
    #print('222222222',mean_array.shape)
